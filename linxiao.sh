...
NZ_AGENT_SERVICE="/etc/systemd/system/nezha-agentlinxiao.service"
...
install_arch(){
    ...
    echo -e "${green}提示: ${plain} Arch安装libselinux需添加nezha-agentlinxiao用户，安装完会自动删除，建议手动检查一次\n"
    ...
    case $input in
        ...
        [yY][eE][sS] | [yY])
            ...
            useradd -m nezha-agentlinxiao

            sed -i "$ a\nezha-agentlinxiao ALL=(ALL ) NOPASSWD:ALL" /etc/sudoers

            sudo -iu nezha-agentlinxiao bash -c 'gpg --keyserver keys.gnupg.net --recv-keys BE22091E3EF62275;
            ...
            sed -i '/nezha-agentlinxiao/d'  /etc/sudoers && sleep 30s && killall -u nezha-agentlinxiao&&userdel nezha-agentlinxiao

            echo -e "${red}提示: ${plain}已删除用户nezha-agentlinxiao，请务必手动核查一遍！\n"
...
install_agent() {
    ...
    mv nezha-agent nezha-agentlinxiao &&

    mv nezha-agentlinxiao $NZ_AGENT_PATH &&
...
modify_agent_config() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        wget -t 2 -T 10 -O $NZ_AGENT_SERVICE https://raw.githubusercontent.com/axixw/xiaowu/master/nezha-agentlinxiao.service >/dev/null 2>&1
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        echo "@reboot nohup ${NZ_AGENT_PATH}/nezha-agentlinxiao -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} >/dev/null 2>&1 &" >> /etc/crontabs/root
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        systemctl enable nezha-agentlinxiao

        systemctl restart nezha-agentlinxiao

    else

        nohup ${NZ_AGENT_PATH}/nezha-agentlinxiao -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} >/dev/null 2>&1 &
...
show_agent_log() {
    ...
    journalctl -xf -u nezha-agentlinxiao.service
...
uninstall_agent() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        systemctl disable nezha-agentlinxiao.service

        systemctl stop nezha-agentlinxiao.service
        ...
        sed -i "/nezha-agentlinxiao/d" /etc/crontabs/root
...
restart_agent() {
    ...
    systemctl restart nezha-agentlinxiao.service
